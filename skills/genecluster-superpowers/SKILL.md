...
**Test sweep:** Public-safe provider-side validation covered every tool above plus 17 extended tools. Result: 25 validated, 3 parked (cblaster + clinker, CLEAN + HIT-EC, HHblits), 2 gated (PlantCyc, ESM-C 6B). Raw run logs are intentionally omitted from this public snapshot; see [`docs/biosymphony-tooling-status.md`](../../docs/biosymphony-tooling-status.md) for the canonical public inventory and re-entry recipes.
...
**Cloud-runtime forward research: 2026-05-11.** Provider-portability reference docs for AWS / GCP / neocloud live at `docs/cloud-runtimes/`. Result: RunPod stays default for both CPU and GPU; clear exceptions documented (NCBI-bandwidth-bound work -> GCP, audit-required work -> AWS, large-scale H100 training -> CoreWeave).
