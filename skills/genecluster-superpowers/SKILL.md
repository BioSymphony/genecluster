...
description: Use when extending the BioSymphony GeneCluster comparative atlas with new tooling, sequence/structure search, biosynthetic-gene-cluster (BGC) detection, synteny ribbons, EC prediction, plant-aware reference databases, interactive viewers, or scientific report rendering. Packages the superpower-discovery survey, the extended testing sweep, and the upstream freshness audit as ready-to-invoke shortcuts so future iterations don't redo the discovery. **Canonical tool-status inventory: `docs/biosymphony-tooling-status.md`** (25 validated / 3 parked / 8 shelved / 2 gated).
---
...
**Single source of truth for tool status:** [`docs/biosymphony-tooling-status.md`](../../docs/biosymphony-tooling-status.md). The status tables in this skill mirror that doc for the originally surveyed subset; the canonical inventory covers the full 25-tool validated set + parked + shelved.

## When to use this skill

- Adding a new species to the atlas and you want better-than-BLAST sequence homology (MMseqs2, Foldseek)
- Detecting BGCs that anchor-windowing missed (plantiSMASH 2.0.4 via the validated v7 boot recipe)
...
- **Adopted (✅)** = validated on RunPod, output integrated into the atlas, canonical path. Just call it.
...
- **Shelved (❓)** = on the roadmap, license-free, never dispatched. Listed in the canonical inventory with effort estimate + re-entry recipe; not key for current work.
- **Gated (⏸️)** = blocked on license application or API key access. Alternative validated where available.

Current status (after the extended testing sweep and upstream freshness audit, for the full 25-tool inventory see [`docs/biosymphony-tooling-status.md`](../../docs/biosymphony-tooling-status.md)):

| Tool | Status | Tier | Validating task | Notes |
|---|---|---|---|---|
| **Quarto 1.9.37** | ✅ ADOPTED | blessed | (adopted) | Quarto atlas book live |
| **plantiSMASH 2.0.4 (v7 boot recipe)** | ✅ ADOPTED | medium-add | validated | Upstream latest is 2.0.4; "v7" is BioSymphony's validated boot iteration, not a release. Raw editable installs had `straight.plugin` discovery blockers; use the non-editable source install + recipe. Detects multiple clusters per chromosome on public plant genomes. AGPL-3.0+. Private use only. |
| **antiSMASH 8.0.4** (bacterial / fungal) | ✅ ADOPTED | (added P12) | validated | Cookbook at `docs/biosymphony-antismash-cookbook.md`. Mambaforge + cpu5g. No `--taxon plants`. |
| **DeepBGC** (plant BGC default) | ✅ ADOPTED | (added during validation testing) | validated | Tens of BGCs detected across multiple chromosomes on a public plant genome |
| **JCVI MCScan** | ✅ ADOPTED | cheap-add | validated | Thousands of pairwise anchors between two public plant species |
| **MMseqs2 iterative** | ✅ ADOPTED | cheap-add | validated | +8, 21 % homologs vs blastp |
| **Foldseek + ProstT5** | ✅ ADOPTED | cross-species evidence layer | validated | Thousands of PDB structural hits returned on a public plant query set |
| **cblaster + clinker** | ⛔ PARKED | cheap-add | validated | Re-entry: stage GenBanks via NCBI Datasets CLI → local cblaster DB → query. |
| **CLEAN + HIT-EC** | ⛔ PARKED | cheap-add | validated | Wrapper brittle; re-entry: bypass `CLEAN_infer_fasta.py`, call `CLEAN.infer.infer_maxsep` directly. **DeepEC / ECPred (validated) is the validated EC alternative.** |
| **HHblits / HHsuite3** | ⛔ PARKED | optional | validated | cpu3g OOM on PDB70; re-entry: cpu5g + pre-stage PDB70 in separate pod. MMseqs2 + Foldseek cover divergent-homolog space. |
| **PlantCyc PMN 16** | ⏸️ GATED | DB-only |. | Academic license required. KEGG / KAAS (validated) covers pathway-completion in the interim. |
| **P450Rdb** | ✅ ADOPTED | DB-only | validated | 3 BIA queries at 100 % identity |
| **Cytoscape.js 3.33.3** | ✅ ADOPTED | frontend |. | Pathway-completion viewer with Fit/Reset/zoom controls |

**Also validated in the extended validation campaigns:** MIBiG 4.0 (validated), ESM-C 300M Synthyra (validated), ESM-2 650M (validated), HMMER (validated), InterProScan (validated), TM-align / mTM-align (validated), MAFFT (validated), IQ-TREE (validated), ColabFold (validated), KEGG mapper / KAAS (validated), EnzymeMap (validated), DiffPaSS (validated), DeepEC / ECPred (validated), igv-reports (validated), pyGenomeTracks (validated), Quarto Dashboards (validated).
...
- [`plantismash-quickstart.md`](references/plantismash-quickstart.md), motif-driven plant BGC detection (use upstream 2.0.4 through the validated v7 boot recipe)
- [`foldseek-prostt5-quickstart.md`](references/foldseek-prostt5-quickstart.md), structure-similarity for convergent enzymes (cross-species evidence layer)
- [`plantcyc-p450rdb-quickstart.md`](references/plantcyc-p450rdb-quickstart.md), P450Rdb validated; PlantCyc still gated on academic license
...
- [`clean-hit-ec-quickstart.md`](references/clean-hit-ec-quickstart.md), bypass `CLEAN_infer_fasta.py`; DeepEC / ECPred (validated) is the validated alternative
...
If the underlying tool isn't installed, each runner exits cleanly with the exact install command. No surprise crashes.
...
When a new tool gets recommended (via a 5-agent discovery dispatch or a new paper), the integration follows the foundation-laying pattern:

1. **Survey & decide**, usually via a multi-Opus-agent discovery dispatch; end with a verdict in the superpower roadmap doc.
...
**Discovery survey:** parallel agent surveys along five angles (sequence/structure search, BGC detection, enzyme function, reporting, comparative-atlas paper styling). Cross-agent consensus picked five high-confidence adds (Quarto adopted; plantiSMASH + cblaster + JCVI + Foldseek + CLEAN queued for adoption when use case lands).

**Test sweep:** Historical provider-side campaigns dispatched every tool above plus 17 extended tools. Result: 25 validated, 3 parked (cblaster + clinker, CLEAN + HIT-EC, HHblits), 2 gated (PlantCyc, ESM-C 6B). Private run logs are intentionally omitted from this public snapshot; see [`docs/biosymphony-tooling-status.md`](../../docs/biosymphony-tooling-status.md) for the canonical public inventory and re-entry recipes.

**Freshness audit:** Verified current release endpoints for the explicitly pinned tools. Corrections applied here: plantiSMASH is upstream 2.0.4 with a BioSymphony v7 boot recipe, not upstream "v7"; antiSMASH current line is 8.0.4; local-only pins for clinker, JCVI, igv-reports, and Cytoscape.js were refreshed.

**Cloud-runtime forward research: 2026-05-11.** 3 parallel Opus agents authored reference docs for porting to AWS / GCP / neocloud, saved at `docs/cloud-runtimes/`. Result: RunPod stays default for both CPU and GPU; clear exceptions documented (NCBI-bandwidth-bound work → GCP, audit-required work → AWS, large-scale H100 training → CoreWeave).

This skill + the linked inventory are the durable shortcuts. Future iterations don't redo the discovery, don't re-survey the cloud landscape, and don't re-learn the parking lessons, they consult this skill, read the inventory, and pick the right starting point.
