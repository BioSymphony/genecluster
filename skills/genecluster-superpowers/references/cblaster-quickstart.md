...
Output flows into `cross-species/bbe-gradient.qmd` (Quarto) as an `<iframe>` embed of `clinker.html`. The cluster CSV joins onto the per-species `clusters-diamond` xlsx sheet via `cblaster_join.py` (planned). Replaces the hand-written gradient table in `data/pathway-species-catalog.tsv` with auto-generated SVG ribbons, the Shan / Sun / Astilbe paper figure style.
