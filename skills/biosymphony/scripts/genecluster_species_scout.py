...
def fetch_ncbi_assembly(species: str) -> AssemblyRecord:
    ...
    if annotation_info.get("name"):
        ...
        # Per memory: NCBI annotation_info flag is unreliable. Probe later by URL.
...
def probe_ngdc_for_genus(genus: str) -> dict[str, NgdcRecord]:
    ...
    Per memory: NGDC GWH plants index is reachable from RunPod; here we use it
...
def render_summary(
    ...
    if seed_queries:
        ...
        lines.append("Inputs must be operator-supplied or derived via a literature-audit Opus agent.")
    ...
    lines.append("")
    lines.append("> **Operator action required:** confirm novelty via 3-Opus literature audit per ")
