...
> **Availability flag:** the SNU host (`p450.riceblast.snu.ac.kr`) returned a TCP timeout during validation. May be transient, but if a re-run needs the FASTA, mirror the validated copy on the campaign RunPod volume rather than fetching live.
