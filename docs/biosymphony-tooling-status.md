...
A second, broader research-agent pass looked beyond pin freshness and asked which new tools, repos, databases, and runtime patterns would make the BioSymphony GeneCluster workflow more useful. The durable shortlist lives in [`biosymphony-next-tooling-radar.md`](biosymphony-next-tooling-radar.md).
...
These tools haven't cut a meaningful release in ≥12 months, so nothing to do, listed here so future operators don't waste a research pass on them.
...
| fair-esm (PyPI) | 2.0.0 (2022-11-01) | Library unmaintained; PyTorch 2.6 `weights_only` patch applied per memory. |
...
| CLEAN | 1.0.1 (2023-03-31) | Release-lite; definitively parked per memory. |
...
A research-agent pass swept the BGC / PLM / structure-prediction / EC / plant-2°-met / atlas-authoring ecosystems for recent releases, repos, and papers. Full table + sources documented separately. The candidates below are the ones that genuinely fill a gap in the current atlas pipeline.
...
### New reference data (Coptis MIA-specific; recorded for internal interest, no campaign planned)

See `docs/<comparative-species-doc-removed>` "New comparative-species candidates" section for the full details. Headline: *Berberis vulgaris* chromosome-level genome + MpAO/MpDAR/MpSOS triad; *Uncaria rhynchophylla* T2T + UrCYP13; Chinese goldthread strictosidine pathway paper (13 functional genes + MsNPF2.6 transporter); interactomics Chinese goldthread paper (6 novel MDRs + charlamine BGC).
...
| **JCVI MCScan** | validated | `cross-species/synteny.qmd` (your atlas chapter) | Thousands of pairwise anchors between two public plant species, dotplot PDF on volume. (NGDC GFF parser fallback). |
...
| Tool | Task | Where in atlas | Notes |
|---|---|---|---|
| **P450Rdb** | validated | `cross-species/p450-classification.qmd` (ythe atlas chapter) | 3 BIA queries match curated P450s at 100 % identity. (FASTA pre-clean). **Availability flag:** the SNU host (`p450.riceblast.snu.ac.kr`) returned a TCP timeout from one of the audit agents. may be transient, but if a re-run needs the FASTA, mirror the validated copy on the provider volume instead of fetching live. |
