...
**Validating run:** `.runtime/<superpowers-launch>/staging/test4b-antismash8-bacterial-demo-boot.sh`.
...
| Docker image | **`condaforge/mambaforge:latest`** + `mamba install -c bioconda antismash` (the `antismash/standalone-lite:8.0.4` official image is **broken on RunPod 2026-05-11/12**, stalls in image pull. See "Image choice" section below.) |
...
The bioconda-installed mambaforge approach is the canonical recipe because the official `antismash/standalone-lite:8.0.4` image stalled in 4 of 4 dispatch attempts on RunPod . All four pods showed the "container never started" pattern (`runtime.uptimeInSeconds=-1, ports allocated but cpu/memory always 0%`). A separate smoke test booted cleanly, which suggests an intermittent image-pull / registry issue. The mambaforge fallback (`condaforge/mambaforge:latest`, ~600 MB, Docker Hub) booted in ~30 seconds on the first try and reached `phase=db_download` within 1 minute.
...
4. **`antismash/standalone-lite:8.0.4` stalls on RunPod (2026-05-11/12).** "Container never started" pattern: `desiredStatus:RUNNING`, `runtime.uptimeInSeconds=-1`, cpu/mem 0% indefinitely. Same image worked on May 10. Use mambaforge + bioconda install instead (more reliable boot, 1 min mamba install penalty).
...
10. **US-KS-2 SECURE can have capacity outages.** When single-DC SECURE returns "no instances available", widen to `["US-KS-2","EUR-IS-1","EU-RO-1","CA-MTL-1"]` rather than falling back to COMMUNITY. COMMUNITY ignores `dataCenterIds` and may route you to US-NC-1 where container-start stalls happen.
...
| Field | Value |
|---|---|
| Provider run id | redacted (success on attempt #4) |
...
(Raw artifacts at `.runtime/<superpowers-launch>/staging/test4b-antismash8-results/`.)

### Capacity recovery sequence: what worked 2026-05-11/12

The validating run took three dispatch attempts. The recipe that finally got scheduled:

1. **Failed:** COMMUNITY widened DC pool → routed to US-NC-1 → container never started for 8-10 min (3 attempts).
2. **Failed:** SECURE pinned to US-KS-2 single-DC → 500 "no instances available."
3. **Worked:** SECURE widened to `["US-KS-2","EUR-IS-1","EU-RO-1","CA-MTL-1"]` with all 4 cpu flavors → instantly scheduled on EU-RO-1.

The lesson: when a single SECURE DC is tight, broaden the SECURE DC list (not the cloud type) before falling back to COMMUNITY. COMMUNITY ignores `dataCenterIds` per the routing memory, so it's not a useful pivot when a specific DC is down.
