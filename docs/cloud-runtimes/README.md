...
3. **Credential injection**, do not rely on provider-injected API keys inside workloads. AWS / GCP avoid this entirely via instance profiles + IMDSv2 / metadata-server. Lambda does not inject anything by default.
...
5. **Persistent volume model**, RunPod network volumes are SECURE-only. AWS EBS attaches anywhere within an AZ; GCP PD attaches within a zone; Lambda's NFS is region-locked. None of the others have RunPod's specific SECURE-vs-COMMUNITY split.
6. **Image-pull stalls**, large container images can stall on some pod hosts. ECR + AWS, Artifact Registry + GCP, and CoreWeave handle large images cleanly. Vast.ai inherits similar risk because the marketplace hosts run Docker via various backends.
7. **NCBI bandwidth**, AWS / GCP get free same-region Open Data mirrors that can be much faster than generic pod egress for SRA-heavy workloads.
...
These RunPod-derived operating lessons carry forward as design constraints regardless of target cloud.
